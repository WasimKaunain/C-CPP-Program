...
class Admin
{
 ...
 void StoreAdminData()
  {
    ofstream fout;
    fout.open("Admin_lib.txt",ios::out);
    if(fout)
    {
...
bool Admin::verifyAdmin(string UID,string PASSWORD)
  {
     ifstream fin;
     fin.open("Admin_lib.txt",ios::in);
     fin.read((char*)this,sizeof(*this));
...
void Admin::AdminLogin()
{
     ...
     if(this->verifyAdmin(userID,password))
      {
...
void Book::GenerateToken(int value,int day,int penalty)
{
  ...
  cout<<"2) Using this Token You can borrow this Book from Library."<<endl;
  ...
  cout<<"1) Print this Token for further use."<<endl;
  cout<<"2) Using this Token You can Return this Book from Library."<<endl;
  cout<<"3) One should have to return the borrowed book within 30 Days."<<endl;
  cout<<"4) After 30 Days, Late fine will be charged for each book on account of 2 Rupee per day."<<endl;
  cout<<"5) Only the borrower will responsible in case of damage or stolen of Books and he will have to pay the amount equal to the price of book."<<endl;
  cout<<endl<<"\t\t\t\t\t\t  T H A N K   Y O U"<<endl;
...
void Student::GetData()
{
    ...
    Student_Pswd=password;
      ...
      if(!PasswordVerifier(Student_Pswd))
    {
  ...
  if(fin)
    {
       ...
       while(!fin.eof())
         {
           if((uid==mobile || uid==EmailID) && password==Student_Pswd)
           {
             flag++;
             break;
           }
