def round_robin_schedule(n):
    ...
    for i in range(half - 1):
        schedule.append(left_schedule[i] + [(x + half, y + half) for x, y in right_schedule[i]])
    
    # Schedule matches between teams in left group and right group
    for day in range(half):
        cross_group_matches = []
        for i in range(half):
            # Rotate the second group for each day
            cross_group_matches.append((i + 1, (i + day) % half + half + 1))
